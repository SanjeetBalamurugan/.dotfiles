...
class AppImage:
    ...
    def install(self) -> None:
        ...
        app_install_path = os.path.join(
            os.environ["HOME"], ".dotfiles", ".app_installs"
        )
        if not os.path.exists(app_install_path):
            os.mkdir(app_install_path)
        ...
        if sys.platform == "linux":
            ...
            os.symlink(
                src=f"/opt/{self.name}/bin/{self.saveName}",
                dst=f"/usr/bin/{self.saveName}",
            )
